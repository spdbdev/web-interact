...
function CreateCampaignPage() {
  ...
  const [navigatedTabs, setNavigatedTabs] = useState([0])
  ...
  navigatedTabs.map((tab) => TABS_COMPONENTS[tab].navigated = true);
  ...
  const changeSelectedTabIndex = (newIndex) => {
    if (isNaN(newIndex)) {
      newIndex = selectedTabIndex + 1;
    }

    if (newIndex > selectedTabIndex && !navigatedTabs.includes(newIndex)) {
      setNavigatedTabs([...navigatedTabs, newIndex]);
    }
