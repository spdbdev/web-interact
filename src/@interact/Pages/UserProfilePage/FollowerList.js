...
export default function FollowerList({open, setOpen}){
    return(
        ...
        >
            ...
            <div onClick={()=>setOpen(false)} style={{cursor:'pointer'}}>X</div>
            </div>
            <div style={{overflowY:'scroll', height:'calc(100% - 30px)'}}>
            {[...Array(100).keys()].map((x, i)=><FollowerListItem />)}
            </div>
      </ReactModal>
...
function FollowerListItem() {
  ...
  return (
    <div style={{display:'flex', paddingTop:10, paddingBottom:10, alignItems:'center', borderBottom:'1px solid #aaa'}}>
        <img src='https://cms-assets.tutsplus.com/uploads/users/810/profiles/19338/profileImage/profile-square-extra-small.png' 
        style={{objectFit:'cover', width:50, height:50, borderRadius:1000}}/>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', width:'100%'}}>
            <div style={{marginLeft:20}}>
                <div style={{fontSize:20}}>Andrew Thompson</div>
                <div style={{fontSize:12, color:'#777'}}>Contributed $50 2 interactions total</div>
            </div>
