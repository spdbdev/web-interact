...
function CompaignPage() {
  ...
  const getCampaignData = async ()=>{
    ...
    setBids(await getFirebaseArray(collection(db, 'campaigns', campaignId, 'bids')));

    setComments(await getFirebaseArray(collection(db, 'campaigns', campaignId, 'comments')))
    setSupporters(await getFirebaseArray(collection(db, 'campaigns', campaignId, 'supporters')))
  }
  ...
  return (
    <div className="CampaignPage">
      ...
      <div style={{display:'flex', marginTop:-20, flexDirection:'row', justifyContent:'space-between', padding:20, paddingTop:20}}>
        {/* <img style={{flex:2, objectFit:'cover', height:520}} alt='placeholderImage' src='https://i.stack.imgur.com/SziYe.png' /> */}
        <iframe width="3200" height="520" src={campaignData?.videoUrl} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
        {num_raffle > 0 ? <Raffle style={{width:300}} campaignData={campaignData}/> : null}
      </div>
      ...
      <div style={{display:'flex', flexDirection:'row', justifyContent:'space-between', paddingLeft:20, paddingRightL:20, paddingBottom:20}}>
        <div style={{flex:2, marginRight:20, marginTop:-20}}>
          <CreatorName   campaignData={campaignData}/>
          <br />
         <CampaignInfo   campaignData={campaignData} comments={comments} campaignId={campaignId} supporters={supporters} />
        </div>
        <div style={{flex:1}}>
