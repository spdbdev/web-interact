  ...
  db.collection("campaigns")
    ...
    .then((snapshot) => {
      ...
      process_giveaway_list(
        campaign_id,
        Number(data.numGiveawayInteractions),
        data.person.id
      );
    });
    ...
    try {
      ...
      for (let document of snapshot.docs) {
        ...
        let docSnap = await db
          .collection("contributionAndGiveawayLossHistory")
          .doc(creator_id)
          .collection("users")
          .doc(document.id)
          .get();
...
exports.autoBidding = functions.firestore
  .document("/campaigns/{campaign_id}/bids/{bid_id}")
  .onWrite(async (snap, context) => {
    ...
    if (original.price >= original.maxBidPrice) {
      console.log(
        "Already at maxBidPrice. ....",
        original.price,
        original.maxBidPrice
      );
      return null;
    }
    ...
    async function perform_auto_bidding(document, index) {
      ...
      const current_index = bids.findIndex(
        (element) => element.id === document.id
      );
      if (current_index == -1) {
        console.log(
          "Error: unable to find index of current user in bids. findIndex is not working"
        );
        return null;
      }
      ...
      bids_coll_ref
        .doc(document.id)
        .set({ price: document.price }, { merge: true });
    }

    function log_bids() {
      console.log(
        "=========================== Sorted Auto Bids Start { ==========================="
      );
      auto_bids.forEach((doc, index) =>
        console.log(
          index,
          doc.auto,
          doc.desiredRanking - 1,
          doc.price,
          doc.maxBidPrice,
          doc.id
        )
      ...
      console.log("--------");
      bids.forEach((doc, index) =>
        console.log(index, "------", doc.price, "------", doc.id)
      );
      console.log(
        "=========================== Sorted Bids End { ==========================="
      );
